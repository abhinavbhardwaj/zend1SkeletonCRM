...
class Upload {
     ...
     * @var integer;
     */
    var $preserve_transparency = true;
    ...
    function process($server_path) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
				/* ***** changes for not to replace space and hypen with underscore in file name */
				
				/*$this->file_dst_name_body = str_replace(array(' ', '-'), array('_','_'), $this->file_dst_name_body) ;
				$this->file_dst_name_body = ereg_replace('[^A-Za-z0-9_]', '', $this->file_dst_name_body) ; */
				
				$this->file_dst_name_body = str_replace(array(' ', '-'), array('-','-'), $this->file_dst_name_body) ;
                /* contd... */
...
<?php
/*
// Below is a sample use of class upload (html not included)
	...
	if($_FILES['file']['name']!="")
	{
		...
		if ($handle->uploaded)
		{
			...
			if ($handle->processed) 
			{
				...
				chmod('uploads/'.$handle->file_dst_name,0777);
				var_dump($handle->file_dst_name);
				echo ' UPLOADED ';
			}
			else 
			{
