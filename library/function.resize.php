 ...
 */
function resize($imagePath,$opts=null,$savefile,$newFileName){
	...
	# start configuration
	$cacheFolder = $savefile; # path to your cache folder, must be writeable by web server
	$remoteFolder = $cacheFolder.'remote/'; # path to the folder you wish to download remote images into

	$defaults = array('crop' => false, 'scale' => false, 'thumbnail' => false, 'maxOnly' => true, 
	   'canvas-color' => $canvasColor, 'output-filename' => false, 
	   'cacheFolder' => $cacheFolder, 'remoteFolder' => $remoteFolder, 'quality' => 150, 'cache_http_minutes' => 20);
	...
	if(false !== $opts['output-filename']) :
        ...
        if(!empty($w) and !empty($h)):
            $newPath = $cacheFolder.$filename.(isset($opts['crop']) && $opts['crop'] == true ? "" : "").(isset($opts['scale']) && $opts['scale'] == true ? "" : "").'.'.$ext;
        elseif(!empty($w)):
