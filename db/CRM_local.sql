...
--
-- Host: localhost
-- Generation Time: Apr 03, 2016 at 11:26 AM
...
CREATE TABLE IF NOT EXISTS `bal_parent_notification` (
  ...
  `notification_type` enum('APPUN','APPAR','APPIN','APPADMRM','REDEEM','UNLOCK','TROPHY','WAGER','IMAGE','LOCK','WEEKLYGOAL','SENDCHALLENGE','PAIR','NEW_SUBJECT','REWARD') DEFAULT NULL COMMENT 'APPUN => app uninstalled, APPAR => app admin removed',
...
CREATE TABLE IF NOT EXISTS `bal_purchase_order` (
  ...
  `payment_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'the date when client will give payment',
